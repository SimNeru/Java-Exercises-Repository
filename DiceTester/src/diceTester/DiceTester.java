...
public class DiceTester {
	...
	public static void main(String[] args) {
		...
		while(condition) {
			...
			if(!superluckyMode) 
			{
				play(dice6, dice12, dice20);
			}
			
			if(superluckyMode || lossCounter==4) 
			{
				cheatMode();
				lossCounter = 0;
				superluckyMode = false;
			}
			...
			{
				superluckyMode = true;
				System.out.println("Activating superlucky");
			}
		}
	...
	public static void play(Dice one, Dice two, Dice three) 
	{
		int dice6Result = one.throwDice();
		int dice12Result = two.throwDice();
		int dice20Result = three.throwDice();
		...
		total += sp(dice6Result, dice12Result, dice20Result);
		
		System.out.println("Total points: " + total + "!");
	}
	
	public static int sp(int one, int two, int three) 
	{
		...
		{
			System.out.println("Not superlucky :/");
			lossCounter++;
			return 0;
		}
	}
